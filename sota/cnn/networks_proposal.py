...
from sota.cnn.model_search_gen_darts_proj import GenDartsNetworkProj
...
parser.add_argument('--data', type=str, default='../../_dataset', help='location of the data corpus')
...
## projection
parser.add_argument('--edge_decision', type=str, default='random',
                    choices=['random', 'reverse', 'order', 'global_op_greedy', 'global_op_once', 'global_edge_greedy',
                             'global_edge_once', 'sample'], help='used for both proj_op and proj_edge')
parser.add_argument('--proj_crit_normal', type=str, default='meco',
                    choices=['loss', 'acc', 'jacob', 'comb', 'synflow', 'snip', 'fisher', 'var', 'cor', 'norm',
                             'grad_norm', 'grasp', 'jacob_cov', 'meco', 'zico', 'ntk', 'gradsign', 'zen'])
parser.add_argument('--proj_crit_reduce', type=str, default='meco',
                    choices=['loss', 'acc', 'jacob', 'comb', 'synflow', 'snip', 'fisher', 'var', 'cor', 'norm',
                             'grad_norm', 'grasp', 'jacob_cov', 'meco', 'zico', 'ntk', 'gradsign', 'zen'])
parser.add_argument('--proj_crit_edge', type=str, default='meco',
                    choices=['loss', 'acc', 'jacob', 'comb', 'synflow', 'snip', 'fisher', 'var', 'cor', 'norm',
                             'grad_norm', 'grasp', 'jacob_cov', 'meco', 'zico', 'ntk', 'gradsign', 'zen'])
...
def main():
    ...
    if args.dataset == 'imagenet':
        model = ImageNetNetworkProj(args.init_channels, n_classes, args.layers, criterion,
                                    spaces_dict[args.search_space], args)
    else:
        model = DartsNetworkProj(args.init_channels, n_classes, args.layers, criterion, spaces_dict[args.search_space],
                                 args)
        model = GenDartsNetworkProj(args.init_channels, n_classes, args.layers, criterion,
                                    spaces_dict[args.search_space],
                                    args, data=torch.randn(size=(1, 3, 32, 32)))
