...
class H5Dataset(Dataset):
    def __init__(self, h5_path, transform=None):
        ...
        self.h5_file = None
        self.length = len(h5py.File(h5_path, 'r')['image'])
    ...
    def __getitem__(self, index):
        ...
        record = (self.h5_file['image'][index], self.h5_file['labels'][index])

        if self.transform:
            x = Image.fromarray(record[0])
            x = self.transform(x)
        else:
            x = torch.from_numpy(record[0])
